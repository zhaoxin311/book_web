  ...
  <div class="login-container">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <el-form-item prop="username">
        ...
        <el-input ref="username" v-model="loginForm.username" placeholder="Username" name="username" type="text" tabindex="1" auto-complete="on" @input="loginForm.username = loginForm.username.replace(regexLogin.username, '')" @keyup.enter.native="$refs.password.focus()" />
      </el-form-item>
      <el-form-item prop="password">
        <span class="svg-container">
          <svg-icon icon-class="password" />
        </span>
        <el-input :key="passwordType" ref="password" v-model="loginForm.password" :type="passwordType" placeholder="Password" name="password" tabindex="2" auto-complete="on" @input="loginForm.password = loginForm.password.replace(regexLogin.password, '')" @keyup.enter.native="$refs.verificationCode.focus()" />
    ...
    <el-dialog title="注册新用户" width="70%" :visible.sync="newUserVisible">
    <el-form ref="registerFrom" :model="registerFrom" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      <div class="title-container">
        <h3 class="title">XXXXX后台管理系统</h3>
      </div>
      <div class="center">
      ...
      <el-form-item prop="username">
        ...
        <el-input ref="username" v-model="registerFrom.username" placeholder="请输入用户名" name="username" type="text" tabindex="1" auto-complete="on" @input="registerFrom.username = registerFrom.username.replace(regexLogin.username, '')" @keyup.enter.native="$refs.password.focus()" />
      </el-form-item>
      <el-form-item prop="password">
        <span class="svg-container">
          <svg-icon icon-class="password" />
        </span>
        <el-input :key="passwordType" ref="password" v-model="registerFrom.password" :type="passwordType" placeholder="请输入密码" name="password" tabindex="2" auto-complete="on" @input="registerFrom.password = registerFrom.password.replace(regexLogin.password, '')" @keyup.enter.native="$refs.confirmPassword.focus()" />
      </el-form-item>
      <el-form-item prop="confirmPassword">
        <span class="svg-container">
          <svg-icon icon-class="password" />
        </span>
        <el-input ref="confirmPassword" v-model="registerFrom.confirmPassword" :type="passwordType" placeholder="请确认密码" name="confirmPassword" tabindex="2" auto-complete="on" @input="registerFrom.confirmPassword = registerFrom.confirmPassword.replace(regexLogin.password, '')" @keyup.enter.native="$refs.email.focus()" />
      </el-form-item>
      ...
      <el-form-item prop="email">
        ...
        <el-input ref="email" v-model="registerFrom.email" placeholder="请输入电子邮件" name="email" type="text" tabindex="1" auto-complete="on" @input="registerFrom.email = registerFrom.email.replace(regexLogin.email, '')" @keyup.enter.native="$refs.password.focus()" />
      </el-form-item>
      ...
      <el-button  type="primary" style="width:48%;margin-bottom:30px;" @click="newUserVisible = false">取消</el-button>
    </el-form>
    </el-dialog>
  </div>
</template>
